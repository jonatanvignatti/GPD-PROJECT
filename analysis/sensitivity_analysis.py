    ...
    def derivative_1d(x, y, axis):
        ...
        if axis == 0:  # over v_w
            ...
            for j in range(n_a):
                ...
                grad[n_v - 1][j] = (y[n_v - 1][j] - y[n_v - 2][j]) / (
                    x[n_v - 1] - x[n_v - 2]
                )
        else:  # over alpha
            for i in range(n_v):
                ...
                grad[i][n_a - 1] = (y[i][n_a - 1] - y[i][n_a - 2]) / (
                    x[n_a - 1] - x[n_a - 2]
                )
        return grad
...
def main():
    data = load_data("/Users/jonatan/GPD-PROJECT/results/peak_trajectories.csv")
    ...
    # Save to JSON
    with open("/Users/jonatan/GPD-PROJECT/results/sensitivity_matrix.json", "w") as f:
